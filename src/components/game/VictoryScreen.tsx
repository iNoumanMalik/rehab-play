...
const STATS_CONFIG = [
  ...
  { key: 'maxCombo' as const, label: 'Best Combo', suffix: 'x', gradient: 'from-purple-500/20 to-pink-500/15', border: 'border-purple-500/40', textColor: 'text-purple-200' },
];
...
export function VictoryScreen({ won, stats, reward, achievements, onPlayAgain, onBack }: VictoryScreenProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg bg-white/[0.04] border border-white/[0.12] rounded-3xl shadow-2xl p-6 sm:p-8 text-center animate-[popIn_0.35s_ease-out]">
        <div className="text-5xl mb-2">{won ? '🏆' : '✅'}</div>
        <h2 className="text-2xl sm:text-3xl font-extrabold bg-gradient-to-r from-white via-white/90 to-white/70 bg-clip-text text-transparent mb-1">
        ...
        {reward && (
          <div className="bg-white/[0.03] border border-white/[0.1] rounded-2xl p-4 mb-5 text-left">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold text-violet-200">
                {reward.leveledUp ? `⬆️ Level ${reward.level}!` : `Level ${reward.level}`}
              </span>
              <span className="text-sm font-bold text-emerald-300">+{reward.xpEarned} XP</span>
            </div>
        ...
        {achievements.length > 0 && (
          <div className="mb-6">
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-white/50 mb-2 text-left">Achievements Unlocked</p>
            <div className="flex flex-wrap gap-2">
              {achievements.map(a => (
                <div key={a.id} className="flex items-center gap-1.5 bg-amber-500/15 border border-amber-500/30 rounded-full pl-1.5 pr-3 py-1">
                  ...
                  <span className="text-xs font-bold text-amber-100">{a.title}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <button
            onClick={onPlayAgain}
            className="px-8 py-3.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white font-extrabold rounded-2xl border border-violet-500/40 shadow-lg transition-all duration-300 cursor-pointer outline-none focus-visible:ring-4 focus-visible:ring-violet-500/50"
          >
            🔁 Play Again
          </button>
          <button
            onClick={onBack}
            className="px-8 py-3.5 bg-white/[0.06] hover:bg-white/[0.12] text-white font-bold rounded-2xl border border-white/[0.12] transition-all duration-300 cursor-pointer outline-none focus-visible:ring-4 focus-visible:ring-white/30"
          >
            Back to Dashboard
          </button>
        </div>
