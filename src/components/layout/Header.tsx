...
export function Header({ isCameraOn, videoReady, isReady, onToggleCamera }: HeaderProps) {
  ...
  return (
    <header className="relative z-20 bg-[var(--color-bg)]/80 backdrop-blur-xl border-b border-[var(--color-border)] sticky top-0">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-[var(--color-text-muted)] font-semibold tracking-widest uppercase mt-0.5">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-2.5 rounded-full text-xs sm:text-sm font-bold border transition-all duration-300 shadow-md cursor-pointer focus-visible:ring-4 focus-visible:ring-violet-500/50 outline-none ${
                isCameraOn
                  ? 'bg-violet-500/20 hover:bg-violet-500/30 text-violet-700 dark:text-violet-200 border-violet-500/40 hover:border-violet-500/60'
                  : 'bg-rose-500/20 hover:bg-rose-500/30 text-rose-700 dark:text-rose-200 border-rose-500/40 hover:border-rose-500/60'
              }`}
            ...
            {isCameraOn && videoReady && (
              <span className="hidden sm:flex items-center gap-2 px-3.5 py-2.5 bg-emerald-500/20 text-emerald-700 dark:text-emerald-200 rounded-full text-xs sm:text-sm font-semibold border border-emerald-500/40 shadow-sm shadow-emerald-500/10">
            ...
            {isCameraOn && videoReady && !isReady && (
              <span className="hidden sm:flex items-center gap-2 px-3.5 py-2.5 bg-amber-500/20 text-amber-700 dark:text-amber-200 rounded-full text-xs sm:text-sm font-semibold border border-amber-500/40 shadow-sm shadow-amber-500/10 animate-pulse">
