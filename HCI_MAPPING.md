...
| Feature | Implemented in | Principle(s) satisfied | Notes |
|---|---|---|---|
| Persistent header chrome (Home via logo, camera toggle, Settings & Accessibility) reachable from every screen while browsing | `src/App.tsx`, `src/components/layout/Header.tsx` | Nielsen #4 Consistency; Shneiderman #1 Strive for consistency | `Header` renders unconditionally outside the page-content branch. **Corrected in Phase F:** once immersive fullscreen play shipped, the Header is visually covered by the Stage during an actual session — the *capability* (Settings/Help/Exit always reachable) is preserved via Stage's own persistent session chrome instead, not via this specific component staying visible. See Phase F. |
...
**Scope note (superseded, see Phase F):** the planned extraction of shared `Button`/`Panel`/`Badge` primitives was originally deferred here to keep this pass fast. It has since shipped in full (Phase F) and is now consumed by 10+ components app-wide.
...
## Phase F — Routing, Immersive Play & Session Chrome

A structured HCI audit (fresh Explore passes across accessibility, interaction design, and a line-by-line re-verification of this document's own claims) found that the premium visual redesign done alongside this phase had, in the process, *regressed* one of Phase D's own claims: going fullscreen for immersive play covered the persistent Header entirely, making Settings unreachable mid-game. That finding drove Phase F's session-chrome work below, alongside the routing/primitives shipped in the same pass.

| Feature | Implemented in | Principle(s) satisfied | Notes |
|---|---|---|---|
| Real URL-based routing (`/`, `/play/:gameId`, `/progress`) replacing state-driven view switching | `src/router.tsx`, `src/App.tsx` (layout route + `useLocation`/`matchPath`) | Nielsen #3 User control & freedom (browser Back/Forward now works); Shneiderman #7 Support internal locus of control | Session state (`currentGameId`) is derived synchronously from the URL — deep-linkable and bookmarkable, without disturbing the persistent camera/pose pipeline (`Stage`'s `<video>` node never remounts across navigation, verified live). |
| Shared `Button`/`Panel`/`Badge` primitives, now fully shipped (supersedes Phase D's "deferred" note) | `src/components/ui/primitives/{Button,Panel,Badge}.tsx`, consumed by 10+ components | Nielsen #4 Consistency; Shneiderman #1 Strive for consistency | Also surfaced and fixed a live duplication: `Onboarding.tsx` had reimplemented its own theme/text-size picker and toggle switch instead of reusing `SettingsMenu`'s `Segmented`/`Switch` — now a single shared implementation (`src/components/ui/settingsOptions.ts`) so the two pickers can't drift apart. |
| Immersive fullscreen play (`Stage.tsx` `fullscreen` mode) **plus** a persistent session-chrome cluster (Exit / Help / Settings) that survives every `GameRunner` phase | `src/components/layout/Stage.tsx` | Nielsen #3 User control & freedom; Nielsen #8 Aesthetic & minimalist design | The chrome cluster is what keeps the Header's Phase D promise true in spirit: Settings opens *upward* from the bottom-right corner (`onDark` prop, `bottom-full`) so it isn't clipped by Stage's `overflow-hidden` — a real bug caught and fixed via a live browser check, not just code review. Exit uses a two-step confirm (see Phase G). |
| Consolidated single-bar HUD (pause + score + health + timer merged into one flush-at-the-top strip) | `src/components/game/HUD.tsx` | Nielsen #1 Visibility of system status; Nielsen #8 Aesthetic & minimalist design | Removed each game's own duplicate canvas-drawn score/hearts band (previously 6 of 7 games independently drew a translucent gradient bar for a single small "Level" label) — one source of truth instead of two competing displays. |

## Phase G — Help, Error Recovery & Progress Visibility

| Feature | Implemented in | Principle(s) satisfied | Notes |
|---|---|---|---|
| Re-openable, non-pausing in-session "How to Play" overlay (per-game instructions, a plain-language explanation of how movement scoring works, and the keyboard-shortcut list) | `src/components/game/HelpOverlay.tsx`, `?` accelerator + Help button in `Stage.tsx` | Nielsen #10 Help & documentation | Onboarding is one-time and per-game intro text is shown once, before calibration — for a rehab tool used across days, this is the mid-session recall path that was previously entirely missing. Instructions live once on `GameMeta.instructions` (`src/games/gameRegistry.ts`), the single source both this overlay and `PlayChrome.tsx` read from. |
| Specific, actionable camera-error messages (permission denied / no device / in use elsewhere), distinguished from generic pose-model failures | `src/components/pose/WebcamFeed.tsx` (`describeWebcamError`), threaded through `Stage.tsx`'s existing error veil | Nielsen #9 Help users recognize, diagnose, and recover from errors | Previously a denied/blocked camera only logged to the console — the user-facing state never updated, leaving an infinite "Setting Up Camera" spinner with no explanation and no recovery path. |
| Two-step "Confirm Exit" / "Confirm Quit" before abandoning an in-progress session | `src/hooks/useTwoStepConfirm.ts`, used by `Stage.tsx`'s Exit button and `GameRunner.tsx`'s "Quit to Dashboard" | Nielsen #5 Error prevention; Shneiderman #6 Permit easy reversal | Deliberately not a full modal — a second click within 3s reduces one-click accidental loss of session effort without adding friction to a genuine, deliberate exit. |
| `/progress` page surfacing full session history and per-exercise trends | `src/pages/ProgressPage.tsx`, `src/core/services/AnalyticsService.ts` (`getHistory()`, already existed) | Nielsen #6 Recognition rather than recall | `AnalyticsService` had persisted complete session history (score/accuracy/duration, last 100 sessions) all along — nothing in the UI ever read it back. Only a live aggregate snapshot (today's level/streak) was shown. For a rehab tool, "is my accuracy improving?" matters more than any single session's number. |
| Keyboard accelerators: `Space` pause/resume, `M` mute/unmute (music+sfx together), `?` toggle Help | `src/components/game/GameRunner.tsx` (Space), `src/components/layout/Stage.tsx` (`M`, `?`) | Shneiderman #2 Enable frequent users to use shortcuts | Documented inside the Help overlay itself so the shortcuts are discoverable, not just present. |
| Landing-page filter chips by focus-area tag, reusing the existing `GameMeta.tag` field | `src/pages/LandingPage.tsx` | Hick's Law (choice-reaction time grows with the number of options); Nielsen #7 Flexibility | A flat, uncategorized grid of 7 (and growing) games doesn't scale — no new taxonomy field was invented, the filter reuses data already on `GameMeta`. |

## Phase H — Accessibility Hardening

| Feature | Implemented in | Principle(s) satisfied | Notes |
|---|---|---|---|
| HUD health bar gets `role="progressbar"` + `aria-valuenow/min/max` + a numeric text alternative ("3/5"), not color/width alone | `src/components/game/HUD.tsx` | WCAG 1.4.1 (Use of Color); WCAG 4.1.2 (Name, Role, Value) | Previously the safe/danger color swap was the *only* signal, with nothing for screen-reader users. |
| Low-time state announced once via a visually-hidden live region on the transition into it, instead of either silence or a per-second announcement | `src/components/game/HUD.tsx` | WCAG 4.1.3 (Status Messages) | A naive `aria-live` on a per-second-updating timer would be unusably noisy; a `useRef` guard fires the announcement exactly once per transition. |
| Route-change focus management (heading receives programmatic focus via `tabIndex={-1}` on mount) and per-route `document.title` | `src/hooks/useDocumentTitle.ts`, `src/pages/LandingPage.tsx`, `src/pages/PlayChrome.tsx`, `src/pages/ProgressPage.tsx` | WCAG 2.4.3 (Focus Order); WCAG 2.4.2 (Page Titled) | New with the router (Phase F) — without this, keyboard/screen-reader users had no orientation cue after navigating. `PlayChrome` gets an `sr-only` heading solely for this announcement, since its visual content sits behind the fullscreen Stage during play. |
| Real Tab/Shift+Tab focus-trap cycling inside the Settings dialog, not just initial-focus + Escape | `src/components/ui/SettingsMenu.tsx` | WCAG 2.4.3 (Focus Order) | Verified live: 40 consecutive Tab presses never move focus outside the open dialog. |
| Decorative emoji/icons marked `aria-hidden="true"` (`GameCard` icon, `Header` logo glyph, HUD pause icon); `ObjectiveBanner` gets `aria-live="polite"` independent of Voice Guidance | `src/components/ui/GameCard.tsx`, `src/components/layout/Header.tsx`, `src/components/game/HUD.tsx`, `src/components/game/ObjectiveBanner.tsx` | WCAG 4.1.2 (Name, Role, Value); WCAG 4.1.3 (Status Messages) | The objective banner is the always-visible "what to do right now" text — previously it was only ever announced via optional, togglable Voice Guidance, with no fallback for screen-reader users who have it off. |
